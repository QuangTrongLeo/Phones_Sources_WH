...
def init():
    ...
    if db_conn:
        # 2.1. Kết nối DB thành công
        # 3. Kiểm tra các bảng cần dùng trong DB, tạo nếu chưa tồn tại
        tables_created = ensure_tables(db_conn)
        if tables_created:
            # 3.1. Nếu các bảng chưa tồn tại, đã được tạo
    ...
    if not configs:
        # 7.1. Không có entrypoints hợp lệ
